                ...
                if (INSTANCE == null) {
                    INSTANCE = Room.databaseBuilder(context.getApplicationContext(), StudentRoomDatabase.class, "student_database").
                        ...
                        public void onCreate(@NonNull SupportSQLiteDatabase db) {
                            ...
                            values.put("studentName", "Pixel");
                            values.put("studentImg", Converters.fromBitmap((BitmapFactory.decodeResource(context.getResources(), R.drawable.cat))));
                            db.insert("students", SQLiteDatabase.CONFLICT_ABORT, values);

                            values.put("studentName", "Bagheera");
                            values.put("studentImg", Converters.fromBitmap((BitmapFactory.decodeResource(context.getResources(), R.drawable.bagheera))));
                            db.insert("students", SQLiteDatabase.CONFLICT_ABORT, values);

                            values.put("studentName", "Ginger");
                            values.put("studentImg", Converters.fromBitmap((BitmapFactory.decodeResource(context.getResources(), R.drawable.ginger))));
                            db.insert("students", SQLiteDatabase.CONFLICT_ABORT, values);
